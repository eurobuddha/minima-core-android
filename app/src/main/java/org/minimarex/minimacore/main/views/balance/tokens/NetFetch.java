...
final class NetFetch {
    ...
    static byte[] get(String url, int maxBytes, int connectMs, int readMs, boolean truncateAtCap) throws Exception {
        return get(url, maxBytes, connectMs, readMs, truncateAtCap,
                u -> (HttpURLConnection) u.openConnection());
    }

    interface ConnectionFactory { HttpURLConnection open(URL url) throws Exception; }

    // Test seam changes transport only; the real host/protocol policy still runs at every hop.
    static byte[] get(String url, int maxBytes, int connectMs, int readMs,
                      boolean truncateAtCap, ConnectionFactory connections) throws Exception {
        String cur = url;
        for (int hop = 0; hop <= MAX_HOPS; hop++) {
            ...
            HttpURLConnection con = connections.open(u);
            con.setInstanceFollowRedirects(false);
            con.setConnectTimeout(connectMs);
            con.setReadTimeout(readMs);
            con.setRequestProperty("User-Agent", "MinimaCore");
            try {
