...
/**
 * Ported from the utxoWallet/Minima wallet: a token is web-validated when the file at its
 * {@code token.webvalidate} URL contains the token's id — proving whoever set it controls that
 * domain. Result is cached per tokenid; fetched once on a worker thread.
 */
public final class WebValidate {
    ...
    public static void ensure(final Activity act, final String tokenid, final String webvalidateUrl, final Runnable onDone) {
    ...
    private static boolean fetchContains(String url, String tokenid) {
        ...
        try {
            ...
            c.setRequestProperty("User-Agent", "MinimaCore");
